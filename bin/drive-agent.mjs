...
ws.onmessage = (e) => {
  ...
  if (m.type === "stderr") return process.stderr.write(`[${el()}] ${m.text}`);
  ...
  if (m.type === "frame") { try { const f = JSON.parse(m.raw); if (f.type === "chunk") process.stdout.write(f.text); else console.log("\n" + m.raw); } catch { console.log("[raw] " + m.raw); } }
};
